          ...
          {tab === 4 && (
            <div style={{ display: 'flex', flexDirection: 'column', gap: '1.25rem' }}>
              <p style={{ margin: 0, fontSize: '0.85rem', color: 'rgba(248,250,252,0.5)', lineHeight: 1.6 }}>
                ...
                Click chips to remove — type in the input and press Enter to add.
              </p>
              ...
              <button
                onClick={() => setRules(rs => [...rs, { label: '', designations: '', prize_rank: prizes.find(p => !p.is_consolation && !p.is_grand_prize)?.rank ?? 2, win_probability: 30 }])}
              ...
              <div style={{ padding: '1rem', background: 'rgba(124,58,237,0.06)', border: '1px solid rgba(124,58,237,0.18)', borderRadius: '0.875rem' }}>
                <p style={{ margin: '0 0 0.625rem', fontSize: '0.75rem', fontWeight: 700, color: '#a78bfa', textTransform: 'uppercase' as const, letterSpacing: '0.06em' }}>Win Probability Summary</p>
                <div style={{ display: 'flex', flexDirection: 'column', gap: '0.3rem' }}>
                  {rules.map((r, i) => {
                    const tPrize = prizes.find(p => !p.is_consolation && !p.is_grand_prize && p.rank === r.prize_rank)
                    const ch = r.designations.split(',').map((d: string) => d.trim()).filter(Boolean)
                    return (
                      <div key={i} style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', fontSize: '0.78rem', color: 'rgba(248,250,252,0.55)' }}>
                        <span style={{ display: 'flex', alignItems: 'center', gap: '0.5rem' }}>
                          ...
                          <span style={{ fontSize: '0.68rem', color: 'rgba(255,255,255,0.25)' }}>{ch.length} titles</span>
                        </span>
