...
export default function NewEventPage() {
  ...
  function switchTab(next) { autoSave(); setTab(next) }
  ...
  function downloadQR() {
    const svg = document.getElementById('new-event-qr')
    if (!svg) return
    const size = 400
        ...
        <div style={{ width: '100%', maxWidth: 480 }}>
          ...
          <div style={{ background: 'rgba(255,255,255,0.04)', border: '1px solid rgba(255,255,255,0.1)', borderRadius: '1.25rem', padding: '2rem', display: 'flex', flexDirection: 'column', gap: '1.25rem' }}>
            ...
            <div>
              ...
              <span style={{ fontSize: '0.72rem', color: 'rgba(248,250,252,0.3)', display: 'block', marginTop: '0.3rem' }}>{origin}/{eventSlug || slugify(name) || 'your-event'}</span>
            </div>
            <button onClick={createEvent} disabled={creating || !name.trim()} style={{ padding: '1rem', border: 'none', borderRadius: '0.875rem', color: '#fff', fontWeight: 800, fontSize: '1rem', marginTop: '0.25rem', background: creating || !name.trim() ? 'rgba(124,58,237,0.3)' : 'linear-gradient(135deg,#7c3aed,#4f46e5)', cursor: creating || !name.trim() ? 'not-allowed' : 'pointer' }}>
    ...
    <div style={{ minHeight: '100vh', background: '#0d0d1f', color: '#f8fafc', fontFamily: 'Inter,sans-serif' }}>
      <header style={{ borderBottom: '1px solid rgba(255,255,255,0.08)', padding: '1rem 2rem', display: 'flex', alignItems: 'center', justifyContent: 'space-between', background: 'rgba(255,255,255,0.02)', position: 'sticky', top: 0, zIndex: 50 }}>
        ...
        <div style={{ display: 'flex', gap: '0.75rem' }}>
          <a href={`/${saved?.slug}`} target="_blank" rel="noreferrer" style={{ padding: '0.6rem 1rem', borderRadius: '0.75rem', border: '1px solid rgba(255,255,255,0.15)', color: 'rgba(248,250,252,0.6)', textDecoration: 'none', fontSize: '0.8rem' }}>👁 Preview</a>
      ...
      <div style={{ maxWidth: 860, margin: '0 auto', padding: '2rem 1.5rem' }}>
        ...
        <div style={{ background: 'rgba(255,255,255,0.04)', border: '1px solid rgba(255,255,255,0.1)', borderRadius: '1rem', padding: '2rem' }}>
          ...
          {tab === 4 && (
            <div>
              <p style={{ margin: '0 0 1rem', fontSize: '0.85rem', color: 'rgba(248,250,252,0.5)' }}>Set win probability per designation group. A roll under the % wins the prize.</p>
          ...
          {tab === 5 && (
            <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', gap: '1.5rem', textAlign: 'center' }}>
              ...
              <button onClick={() => router.push('/admin')} style={{ padding: '0.75rem 2rem', background: 'linear-gradient(135deg,#7c3aed,#4f46e5)', border: 'none', borderRadius: '0.75rem', color: '#fff', fontWeight: 700, cursor: 'pointer', fontSize: '0.9rem' }}>Back to Admin →</button>
            </div>
          )}
        </div>
        ...
        {tab < 5 && (
          <div style={{ display: 'flex', justifyContent: 'space-between', marginTop: '1.5rem' }}>
            <button onClick={() => switchTab(Math.max(0, tab - 1))} disabled={tab === 0} style={{ padding: '0.75rem 1.5rem', borderRadius: '0.75rem', border: '1px solid rgba(255,255,255,0.15)', background: 'transparent', fontSize: '0.875rem', cursor: tab === 0 ? 'not-allowed' : 'pointer', color: tab === 0 ? 'rgba(248,250,252,0.2)' : 'rgba(248,250,252,0.6)' }}>← Previous</button>
            {tab < 4
