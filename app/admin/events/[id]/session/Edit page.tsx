...
export default function EditEventPage() {
  ...
  useEffect(() => {
    fetch('/api/admin/events').then(r => r.json()).then(data => {
      ...
      setBg(ui.bgColor || '#0a0a1a')
      setBg2(ui.bgColor2 || '#312e81')
      setAccent(ui.accentColor || '#f59e0b')
      setHeading(ui.heading || '')
      setLogo(ui.logoUrl || '')
      setFooter(ui.footerText || '')
      if (ev.prizes?.length) {
        setPrizes(ev.prizes.map((p: Prize) => ({
          rank: p.rank, name: p.name, description: p.description || '',
          image_url: p.image_url || '', is_consolation: p.is_consolation, is_grand_prize: p.is_grand_prize,
        })))
      }
  ...
  if (loading) return (
    <div style={{ minHeight: '100vh', background: '#0d0d1f', display: 'flex', alignItems: 'center', justifyContent: 'center', color: 'rgba(248,250,252,0.4)', fontFamily: 'Inter,sans-serif' }}>
      Loading…
    </div>
  )
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: '0.75rem' }}>
              {prizes.map((p, i) => (
                <div key={i} style={{ display: 'grid', gridTemplateColumns: 'auto 1fr 1fr', gap: '0.5rem', alignItems: 'center', padding: '0.75rem', background: 'rgba(0,0,0,0.2)', borderRadius: '0.75rem' }}>
                  ...
                  <input style={{ ...F, padding: '0.5rem 0.75rem' }} value={p.image_url} placeholder="Image URL"
